from get_features_table import df_to_sql, get_user_post_features
...
if __name__ == "__main__":
    ...
    # # Create dataframe with features for NN learning
    # data, cat_columns, post = get_vector_df(df_post_128d)
    data = pd.read_csv('df_to_learn_mlp_128d_post_embedd.csv', sep=';')

    print(f'Positive class share: {data.target.sum()/len(data):.5f}')
    ...
    # If model is presented locally already - additional learning
    if not IS_NEW_MODEL:
    ...
    whole_train_valid_cycle(model,
                            N_EPOCHS,
                            'Learning make post recommendations',
                            train_loader,
                            test_loader,
                            device,
                            optimizer)
    ...
    #
    # # Prepare user features dataframes and list of original master df columns
